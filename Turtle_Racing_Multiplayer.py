...
def start_race(ax,ay,ts):
    ...
    while starter.isvisible()==True:
        starter.color("black")
        starter.color("green")
def run_forrest_run(forrest,key):
    def forrest_runs():
        turtle.delay(0)
        forrest.fd(4)
        #jenny.fillcolor("red")
    ...
    turtle.onkeyrelease(forrest_runs,str(key))
...
def race(n,l,difficulty):
    ...
    while play==1:
        ...
        clicker.st()
        racer_key='space'
        run_forrest_run(racer,racer_key)
        for i in range(1,n):
            run_forrest_run(a[key[i]],str(i))
            turtle.listen()
        while racer.xcor()!=(goal_zone):
            turtle.delay(1)
            a[key[0]].fd(1)
            #for t in range(n):
